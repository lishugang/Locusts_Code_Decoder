...
class Region_proposal():
    ...
    def img_process(self, img):
        ...
        # For double-peak img threshold. (0 is ok)
        _, otsu_img = cv2.threshold(img_gray, 0, 255, cv2.THRESH_OTSU)
        contours, _ = cv2.findContours(otsu_img, cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE)
        noise_otsu_white_area = []
        for contour in contours:
            area = cv2.contourArea(contour)
            if area < 500:
                noise_otsu_white_area.append(contour)
            else:
                continue
        cv2.fillPoly(otsu_img, noise_otsu_white_area, 0)
        dla_kernel = np.ones((20, 20), np.uint8)
        thresh_ots = cv2.dilate(otsu_img, dla_kernel)
